        ...
        {!isLoading && !error && filteredPatients.length > 0 && (
          <div className="flex flex-col sm:flex-row justify-between items-center p-4 border-t border-gray-200 gap-4">
            <div className="text-sm text-gray-500">
              Showing <span className="font-semibold">{indexOfFirstItem + 1}</span> to <span className="font-semibold">{Math.min(indexOfLastItem, filteredPatients.length)}</span> of <span className="font-semibold">{filteredPatients.length}</span> Results
            </div>
            <div className="flex items-center gap-1">
              ...
              {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                // Logic to show generic window of pages around current would be better for many pages, 
                // but simple 1-5 or simple lists works for MVP
                let pageNum = i + 1;
